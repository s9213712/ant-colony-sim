...
python3 experiments/paper_conditions_probe.py \
  --seeds 1-3 \
  --output outputs/paper_conditions_v2.csv \
  --json-output outputs/paper_conditions_v2.json \
  --report-output outputs/paper_conditions_report_v2.md
```
...
- 這輪新增文獻指出兩個新的模型缺口：大量死亡後的覓食韌性偏弱；negative pheromone 目前只是局部排斥場，還不像雙費洛蒙模型中的可學習「禁止路徑」記憶。
