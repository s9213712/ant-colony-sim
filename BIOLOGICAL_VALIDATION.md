...
python3 experiments/paper_conditions_probe.py \
  --seeds 1-3 \
  --output outputs/paper_conditions_v4.csv \
  --json-output outputs/paper_conditions_v4.json \
  --report-output outputs/paper_conditions_report_v4.md
```
...
| 文獻 / 條件 | 狀態 | 觀測摘要 | 仍缺什麼 |
| --- | --- | --- | --- |
| Perna et al. 2012 / local pheromone trail | partial | 平均 `food_trips=49.333`、`food_pheromone=47550.0`，可形成 food trail | 尚未輸出左右感知濃度與轉角，不能擬合 Weber-law turning response |
...
| Deneubourg/Goss/Beckers double bridge | partial | upper bias 在 `0.667` runs 被選中，平均 dominance `0.1543` | 正回饋/橋幾何尚未貼近經典 branch-choice probability curve |
...
| Shiraishi et al. 2018 / diverse stochasticity | partial | diverse `trips_vs_initial=0.3873`，略高於 low `0.3732`，但 high `0.4973` 更高 | 尚未重建文獻中的 environment-dependent optimum distribution |
| Malickova/Yates/Bodova 2015 / two-cue adaptation proxy | partial | relocation 有部分適應，washout 能清除 food pheromone | 不是該文 exact two-pheromone stochastic model，缺 synchronization metric |
...
| Afek/Kecher/Sulamy 2015 / fail-stop foraging | pass | 大量死亡後 food trips 仍非零，resilience ratio `0.3913` | 仍缺 algorithmic lower-bound 對照，不能作 proof-level validation |
| Jimenez-Romero et al. 2015 / negative pheromone | partial | avoid field 將 hazard occupancy ratio 降到 `0.8803`，但效果弱 | 缺個體學習、短/長效雙費洛蒙分工、明確 forbidden-path 記憶 |
...
| Jackson & Chaline 2007 / food quality recruitment | pass | high-quality trips `12.667` > low-quality trips `8.333`，平均採集品質 `1.3243` | 缺物種專屬蔗糖濃度校準與直接 trail-laying event counts |
| Avanzi, Lisart & Detrain 2024 / necrophoresis cleanup | pass | 巢區屍體由 `36.0` 降到 `1.333`，平均 disposed corpses `31.0` | 缺病原狀態、屍體年齡化學曲線與 colony interaction network 驗證 |
...
`experiments/evaluate_literature_corpus.py` 逐篇讀取 `outputs/literature_corpus_100.json`，並把每一篇對應到目前 `outputs/paper_conditions_v4.json` 的 simulation evidence。輸出：
...
| 狀態 | 數量 | 解讀 |
| --- | ---: | --- |
| pass | 8 | 有 exact paper-condition，且目前定性方向符合 |
| partial | 73 | 有 exact condition 或 category proxy，但缺 paper-specific 數據/機制 |
| not_covered | 7 | 目前驗證套件或模型缺必要條件 |
...
1. `brood_microclimate_needed`：建立育幼微氣候 condition，測溫濕度與 brood survival proxy。
2. `nest_relocation_needed`：建立 nest-site quorum / relocation condition。
3. `active_misleading_pheromone`：把 static fake trail 改成 active detractor agents。
4. `sensing_turning_log`：輸出逐步感知/轉向資料，校準 Weber/tropotaxis。
5. `necrophoresis_calibration`：在現有 corpse cleanup 基礎上加入 corpse-age chemistry、pathogen state、interaction network。
...
| 優先級 | 數量 | 用途 |
| --- | ---: | --- |
| P0_missing_biology_condition | 7 | 生物學相關，但目前模型/測試條件不足，優先補 |
| P1_exact_condition_partial | 6 | 已有 exact paper condition，但仍缺關鍵量測或機制 |
| P2_proxy_only | 67 | 只有泛化 proxy，需逐篇轉成專用 condition |
| P3_algorithmic_reference_only | 32 | robotics / ACO / 工程類比，只作靈感或演算法參考 |
| total | 112 | 所有尚未 fully simulated 的文獻 |
...
- necrophoresis corpse-age chemistry、pathogen state 與 worker interaction network 校準。
