...
| Jackson & Chaline 2007 / food quality recruitment | pass | counterbalanced 條件下平均採集品質 `1.2043`，high-quality source pheromone `137.0` > low-quality `73.167` | 缺物種專屬蔗糖濃度校準與直接 trail-laying event counts |
...
| Baudier et al. 2019 / brood microclimate | pass | heat-dry stress `1.8` > stable `0.0`；cold pupal brood chamber 比 cold larval 高 `1.467°C` | 缺 fitted metabolic heat budget、巢址幾何與物種專屬 brood survival curve |
| Pratt et al. 2002 / nest relocation quorum | pass | high-quality site visits `84.823` > low-quality `0.0`，quorum/redeployment 完成 | 缺 tandem running、搬運軌跡、巢容積與 quorum threshold 實測校準 |
...
- v5 新增 brood microclimate 與 nest relocation/quorum 的通用規則與驗證條件。實驗腳本只能改參數、初始條件與環境狀態；個體決策、task/state transition、費洛蒙規則必須由同一套基礎模型產生，不能為單篇文獻注入例外行為。
...
| 狀態 | 數量 | 解讀 |
| --- | ---: | --- |
| pass | 13 | 有 exact paper-condition，且目前定性方向符合 |
...
1. `sensing_turning_log`：輸出逐步感知/轉向資料，校準 Weber/tropotaxis。
2. `active_misleading_pheromone`：把 static fake trail 改成 active detractor agents。
3. `flow_density_log`：輸出 trail segment speed/flow/density 曲線。
...
| 優先級 | 數量 | 用途 |
| --- | ---: | --- |
| P1_exact_condition_partial | 4 | 已有 exact paper condition，但仍缺關鍵量測或機制 |
| P2_proxy_only | 69 | 只有泛化 proxy，需逐篇轉成專用 condition |
| P3_algorithmic_reference_only | 34 | robotics / ACO / 工程類比，只作靈感或演算法參考 |
| total | 107 | 所有尚未 fully simulated 的文獻 |

目前沒有 `P0_missing_biology_condition`。剩下工作不是「完全不可測」，而是把 exact partial 與 proxy-only 文獻逐篇提高到可量化校準。
