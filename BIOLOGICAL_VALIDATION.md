...
下一步：不是再加特例規則，而是把目前的 aggregate log 擴充成可下載的 per-step individual trajectory/sensing series，並用 digitized paper curves 擬合 tropotaxis response function。
...
| 文獻 / 條件 | 狀態 | 觀測摘要 | 仍缺什麼 |
| --- | --- | --- | --- |
| Perna et al. 2012 / local pheromone trail | pass | 平均 `food_trips=123.333`、`food_pheromone=86018.0`、gradient alignment `0.998`，且 trail segment flow `165.6976` | 已有 aggregate sensing/turning 與 segment metrics；仍缺 digitized curve fitting |
| Ramirez et al. 2018 / tropotaxis | pass | 與 Perna 條件共享 gradient-turn 與 segment-flow metrics | 缺逐步個體 trajectory series 與方程式級擬合 |
...
| Deneubourg/Goss/Beckers double bridge | partial | biased branch 未穩定被選中，平均 dominance `0.4678` | 正回饋方向與橋幾何尚未貼近經典 branch-choice probability curve |
...
- 下一個最重要的科學改進不是增加更多 UI，而是把現有 aggregate metrics 推進到可校準資料輸出：per-step sensing/turning/trajectory log、branch choice probability curve fitting、body-contact/lane-discipline calibration。
...
- v5 新增 brood microclimate、nest relocation/quorum、aggregate sensing/turning、segment flow-density、task-switch/contact metrics 與 misleading-trail avoid learning 的通用規則/量測。實驗腳本只能改參數、初始條件與環境狀態；個體決策、task/state transition、費洛蒙規則必須由同一套基礎模型產生，不能為單篇文獻注入例外行為。
...
1. `per_step_sensing_trajectory_log`：輸出逐步感知/轉向/位置資料，校準 Weber/tropotaxis。
2. `branch_choice_curve_fitting`：digitize double-bridge branch-choice curves，檢查 probability/time course。
